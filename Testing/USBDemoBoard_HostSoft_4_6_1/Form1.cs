...
namespace USBDemoBoard
{
	...
	public partial class Form1 : Form
	{
        ...
        //@1
        const byte LED4_ON = 0x01;
        const byte LED4_OFF = 0x00;
        const byte LED5_ON = 0x01;
        const byte LED5_OFF = 0x00;
        const byte LED0_ON = 0x01;
        const byte LED0_OFF = 0x00;
		...
		private void tmrUSB_Tick(object sender, EventArgs e)
		{
            ...
            //@2
            if (rdoLED4On.Checked == true)
            {
                USBObject.fromHostToDeviceBuffer[4] = LED4_ON;
            }
            else if (rdoLED4Off.Checked == true)
            {
                USBObject.fromHostToDeviceBuffer[4] = LED4_OFF;
            }

            if (rdoLED5On.Checked == true)
            {
                USBObject.fromHostToDeviceBuffer[5] = LED5_ON;
            }
            else if (rdoLED5Off.Checked == true)
            {
                USBObject.fromHostToDeviceBuffer[5] = LED5_OFF;
            }

            if (rdoLED0On.Checked == true)
            {
                USBObject.fromHostToDeviceBuffer[9] = LED0_ON;
            }
            else if (rdoLED0Off.Checked == true)
            {
                USBObject.fromHostToDeviceBuffer[9] = LED0_OFF;
            }
